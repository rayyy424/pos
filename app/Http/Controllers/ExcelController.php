...
class ExcelController extends Controller
{
    ...
    // return view('excel', ['me' => $me, 'claim' =>$claim,'receipts' =>$receipts, 'Id' =>$id, 'user' =>$user,'claimdetail' => $claimdetail,'projects' =>$projects,'projectcodes' =>$projectcodes, 'options' =>$options,'mylevel' => $mylevel,'total' => $total]);
	// }
	...
	public function ExcelClaim($id,$userid,$filename = null,$sheetname = null)
	{
		...
		Excel::create($filename, function($excel) use($sheetname,$user,$me,$claim,$receipts,$id,$claimdetail,$projects,$projectcodes,$options, $mylevel,$total){

            $excel->sheet($sheetname, function($sheet) use($user,$me,$claim,$receipts,$id,$claimdetail,$projects,$projectcodes,$options, $mylevel,$total){

                $sheet->loadView('excelclaim', array('me' => $me, 'claim' =>$claim,'receipts' =>$receipts, 'Id' =>$id, 'user' =>$user,'claimdetail' => $claimdetail,'projects' =>$projects,'projectcodes' =>$projectcodes, 'options' =>$options,'mylevel' => $mylevel,'total' => $total));
	...
	public function ExcelTimesheet($id,$start = null, $end = null,$type,$filename = null,$sheetname = null)
	{
		...
		'timesheets.Time_In','timesheets.Time_Out','timesheets.Allowance','timesheets.Leader_Member','timesheets.Next_Person','projectcodes.Project_Code','projects.Project_Name','timesheets.Site_Name','timesheets.State','timesheets.Work_Description','timesheets.Reason','timesheets.Remarks','approver.Name as Approver','timesheetstatuses.Status','timesheetstatuses.Comment','timesheetstatuses.updated_at as Review_Date')
		...
		Excel::create($filename, function($excel) use($sheetname, $me,$id,$user,$start,$end,$timesheetdetail,$projects,$projectcodes,$options,$mylevel,$total){

            $excel->sheet($sheetname, function($sheet) use($me,$id,$user,$start,$end,$timesheetdetail,$projects,$projectcodes,$options,$mylevel,$total){

                $sheet->loadView('exceltimesheet', array('me' => $me, 'UserId' =>$id, 'user' =>$user,'start'=>$start,'end'=>$end,'timesheetdetail' => $timesheetdetail,'projects' =>$projects,'projectcodes' =>$projectcodes, 'options' =>$options,'mylevel' => $mylevel,'total' => $total));
