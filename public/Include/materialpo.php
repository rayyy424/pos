...
// DataTables PHP library
  ...
  $editor=Editor::inst( $db,'materialpo')
    ...
    // ->leftjoin('(SELECT inventoryvendor.CompanyId,MaterialId,vendorId,Sum(materialrequest.Qty*inventoryvendor.Item_Price) as total 
    // from materialrequest  left join inventoryvendor on inventoryvendor.Id = materialrequest.vendorId
    // left join inventories on inventories.Id = materialrequest.InventoryId
    // where inventories.Type <> "MPSB" and inventories.Type <> "Transport"
    // group by  materialrequest.MaterialId,inventoryvendor.CompanyId
    // ) as request','request.CompanyId = materialpo.VendorId AND request.MaterialId = materialpo.MaterialId','','')
    // ->leftjoin('(SELECT Type,PoId from materialpoitem group by Type)','')
