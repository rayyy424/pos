 ...
 if (( isset( $_POST['action'] )) && ( $_POST['action'] == 'edit'))
 {
   ...
   foreach ($_POST['data'] as $key => $value) {
     ...
     if (isset( $_POST['data'][$key]['claims']['Total_Expense']) || isset( $_POST['data'][$key]['claims']['Petrol_SmartPay']) || isset( $_POST['data'][$key]['claims']['Advance']))
     {
        ...
        $_POST['data'][$key]['claims']['Total_Amount']=$_POST['data'][$key]['claims']['Total_Expenses']-$_POST['data'][$key]['claims']['Petrol_SmartPay']-$_POST['data'][$key]['claims']['Advance']-$_POST['data'][$key]['claims']['Summon'];
...
// DataTables PHP library
  ...
  $editor=Editor::inst( $db, 'claimstatuses')
      ...
      Field::inst( 'claims.Project_Code_Id' ),
      Field::inst( 'projectcodes.Project_Code' ),
      Field::inst( 'claims.ProjectId' ),
     ...
     Field::inst( 'claims.Rate' ),
      ...
      Field::inst( 'claims.Summon' )->validator( 'Validate::numeric' ),
      Field::inst( 'allowance.Allowance' )->validator( 'Validate::numeric' ),
      Field::inst( 'allowance.Monetary_Comp' )->validator( 'Validate::numeric' ),
    ...
    ->leftJoin('(Select Date,sum(Allowance) as Allowance,sum(Monetary_Comp) as Monetary_Comp from timesheets where UserId='.$_POST['UserId'].' AND Date in (Select claims.Date from claims where ClaimsheetId='.$_POST['Id'].') GROUP By Date) as allowance','allowance.Date','=','claims.Date AND claims.Id in (Select min(claims.Id) from claims left join claimsheets on claims.ClaimsheetId=claimsheets.Id Where ClaimsheetId='.$_POST['Id'].' Group By Date)')
    ->where( function ( $q ) {
