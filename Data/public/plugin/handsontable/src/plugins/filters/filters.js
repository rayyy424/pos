...
import {CONDITION_NONE, OPERATION_AND, OPERATION_OR, OPERATION_OR_THEN_VARIABLE} from './constants';
...
class Filters extends BasePlugin {
   ...
   * hot.getPlugin('filters').addCondition(1, 'begins_with', ['de']);
   * hot.getPlugin('filters').addCondition(1, 'not_contains', ['ing']);
   * hot.getPlugin('filters').filter();
   *
   * // If you want to add filter conditions with OR operator you can use condition "by_value"
   ...
   *
   * @private
   * @param {Object} conditionState
   */
  updateComponents(conditionState) {
    if (!this.dropdownMenuPlugin.enabled) {
      return;
    }

    const conditions = conditionState.editedConditionStack.conditions;
    const column = conditionState.editedConditionStack.column;

    if (conditions.length === 0) {
      this.components.get('filter_by_condition').updateState(null, column);
      this.components.get('filter_by_condition2').updateState(null, column);
      this.components.get('filter_by_value').updateState(conditionState);
      this.components.get('filter_operators').updateState(this.conditionCollection.columnTypes[column], column);

    } else if (conditions.length > 3 || conditions.filter((condition) => condition.name === 'by_value').length > 1 ||

      conditions.filter((condition) => condition.name !== 'by_value').length > 2) {
      ...
      if (conditions[0] && conditions[0].name === 'by_value') {
        this.components.get('filter_by_condition').updateState(null, column);
        this.components.get('filter_by_condition2').updateState(null, column);

      } else if (conditions[1] && conditions[1].name === 'by_value') {
        this.components.get('filter_by_condition').updateState(conditions[0], column);
        this.components.get('filter_by_condition2').updateState(null, column);

      } else {
        this.components.get('filter_by_condition').updateState(conditions[0], column);
        this.components.get('filter_by_condition2').updateState(conditions[1], column);

        this.showComponentForParticularColumn(this.components.get('filter_by_condition2'), column);
      }
