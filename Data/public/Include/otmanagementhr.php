...
// DataTables PHP library
  ...
  $editor=Editor::inst( $db, 'timesheets')
    ...
    ->on('postEdit', function ($editor, $id, $values, $row) use ($db) {
        ...
        $timesheet = $db->raw()
           ->bind(':Id', $id)
           ->exec("SELECT timesheets.UserId, timesheets.Date, users.Position, users.HolidayTerritoryId FROM timesheets LEFT JOIN users ON users.Id = timesheets.UserId WHERE timesheets.Id = :Id")
           ->fetch();

        $userId = $timesheet['UserId'];
        $userPosition = $timesheet['Position'];
        ...
        if (strtoupper($userPosition) == 'MECHANIC') {
            $earlyTimeIn = $db->raw()
               ->bind(':UserId', $userId)
               ->bind(':Date', $date)
               ->exec("
