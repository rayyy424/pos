...
// DataTables PHP library
  ...
  $editor=Editor::inst( $db, 'timesheets')
    ...
    ->on('postEdit', function ($editor, $id, $values) use ($db) {
        ...
        $timesheet = $db->raw()
           ->bind(':Id', $id)
           ->exec("SELECT timesheets.UserId, timesheets.Date, users.HolidayTerritoryId FROM timesheets LEFT JOIN users ON users.Id = timesheets.UserId WHERE timesheets.Id = :Id")
           ->fetch();
    ...
    if($_GET['UserId']=="855"){
      ...
      $editor
          ->where( function ( $q ) {
              $q->where('(timesheets.OT1','0  OR timesheets.OT2>0 OR timesheets.OT3>0)', ">", false );
          } );
    }
    ...
    if (isset( $_GET['Start'])) {
      ...
      // $editor
      // ->where( function ( $q ) {
      //   $q->where( 'users.Resignation_Date', '""', '=', false );
      // } );

      // $editor
      // ->where( function ( $q ) {
      //   $q->where( 'users.Entitled_for_OT', '"Yes"', '=');
      // } );
