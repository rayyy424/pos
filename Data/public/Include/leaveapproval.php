...
// DataTables PHP library
  ...
  $editor=Editor::inst( $db, 'leaves')
    ...
    // ->leftJoin('leaves', 'leaves.Id', '=', 'leavestatuses.LeaveId')
  ...
  // } );
      ->where( function ( $q ) {
    $q->where( 'leavestatuses.Id', NULL, 'IS NOT', false );
      } );
      ...
      $editor
          ->where( function ( $q ){
              $q->where( 'str_to_date(leaves.Start_Date,"%d-%M-%Y")', 'BETWEEN (str_to_date("'.$_GET['Start'].'","%d-%M-%Y")) AND (str_to_date("'.$_GET['End'].'","%d-%M-%Y"))', "", false )
                ->where( 'str_to_date(leaves.End_Date,"%d-%M-%Y")', 'BETWEEN (str_to_date("'.$_GET['Start'].'","%d-%M-%Y")) AND (str_to_date("'.$_GET['End'].'","%d-%M-%Y"))', "", false );
          } );
    }
    ...
    //   $editor
    //       ->where( function ( $q ){
    //           $q->where( 'str_to_date(leaves.End_Date,"%d-%M-%Y")', 'BETWEEN (str_to_date("'.$_GET['End'].'","%d-%M-%Y")) AND (str_to_date("'.$_GET['End'].'","%d-%M-%Y"))', "", false );
    //       } );
    // }
    ...
    if (isset( $_GET['Type'])) {
      $editor
          ->where( function ( $q ){
              $q->where('leaves.Leave_Type', $_GET['Type'],'=');
          } );
    }
