...
@section('datatable-js')
      ...
      function initMap(lat,long) {
          @if(count($timesheetdetail)>0 && $timesheetdetail[0]->Latitude_In!="")
            var mapOptions = {
      ...
      function myfunction(type,latitude, longitude){

          for (var i = 0; i < gmarkers.length; i++) {

            if(type=="In" && latitude>0)
            {
...
@section('content')
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Attendance Tracker</li>
      </ol>
    </section>
    ...
    <section class="content">
      <div class="row">
       ...
       <div class="col-md-12">
         <div>
             <img src="{{ asset('img/engineer_in.png') }}"><span>&nbsp;&nbsp;Time In</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
             <img src="{{ asset('img/engineer_out.png') }}"><span>&nbsp;&nbsp;Time Out</span>
             <br>
             <br>
         </div>
       </div>
  ...
  function convertTime(time)
  {
    ...
    return converted;
      ...
      function refresh()
      {
        ...
        window.location.href ="{{ url("/engineerlocationtracking") }}/"+arr[0]+"/"+arr[1]+"/"+includeresigned;
      ...
      $(document).ready(function() {
        $(document).on('click', '.otw', function(e) {
          ...
          if(route)
          {
            ...
            $.ajaxSetup({
              ...
              $.ajax({
                  url: "{{ url('/otwgetpoints/') }}" +  "/" + id,
                  method: "GET",
                  success: function(response){
                    for (var i=0; i<omarkers.length; i++) {
                          omarkers[i].setMap(null);

                        }
                    response.item.forEach(function(element) {
                        addMarker(element.Latitude, element.Longitude, element.Time)
                      routePoints.push(new google.maps.LatLng(element.Latitude,element.Longitude));
                      })
                    ...
                    route.setMap(map);
                    map.setZoom(17);
                    var latLng = new google.maps.LatLng(response.item[0].Latitude, response.item[0].Longitude);
                    map.panTo(latLng);
                  }
              });
        });
