      ...
      <script type="text/javascript" language="javascript" class="init">
         ...
         $(document).ready(function() {
           ...
           listeditor = new $.fn.dataTable.Editor( {
                   ajax: {
                      "url": "{{ asset('/Include/filingsystem.php') }}",
                      "data": {
                          "type": "{{ $type }}"
                      }
                    },
                   table: "#list",
                   idSrc: "filingsystem.Id",
                   fields: [

                          {
                                   label: "Type:",
                                   name: "filingsystem.Type",
                                   type:"readonly",
                                   def: "{{ $type }}"
                           },{
                                   label: "Date:",
                                   name: "filingsystem.Date",
                                   type:   'datetime',
                                   def:    function () { return ""; },
                                   format: 'DD-MMM-YYYY',
                                   attr: {
                                   ...
                                   label: "Destruction_Date:",
                                   name: "filingsystem.Destruction_Date",
                                   type:   'datetime',
                                   def:    function () { return ""; },
                                   format: 'DD-MMM-YYYY',
                                   attr: {
                                   ...
                                   label: "Description:",
                                   name: "filingsystem.Description",
                                   type:"textarea"

                           },{
                                   label: "Box_No:",
                                   name: "filingsystem.Box_No"

                           },{
                                   label: "File_Type:",
                                   name: "filingsystem.File_Type"

                           },{
                                   label: "Box_File:",
                                   name: "filingsystem.Box_File"

                           },{
                                   label: "Company:",
                                   name: "filingsystem.Company",
                                   type:  'select2',
                                   options: [
                                       { label :"", value: "" },
                                   ...
                                   label: "File_No:",
                                   name: "filingsystem.File_No"

                           },{
                                   label: "Year:",
                                   name: "filingsystem.Year",
                                   type: "select2",
                                   options: [
                                    { label :"", value: "" },
           ...
           listtable = $('#list').dataTable( {
                   ...
                   columns: [
                     {data: null, title:"No"},
                     {data:"filingsystem.Id", title:"Id"},
                     {data:'filingsystem.Type', title:"Type"},
                     {data:'filingsystem.Box_No', title:"Box No"},
                     {data:'filingsystem.Box_File', title:"Box File"},
                     {data:'filingsystem.File_Type', title:"File Type"},
                     {data:'filingsystem.Company', title:"Company"},
                     {data:'filingsystem.Description', title:"Description"},
                     {data:'filingsystem.File_No', title:"File No"},
                     {data:'filingsystem.Date', title:"Date"},
                     {data:'filingsystem.Year', title:"Year"},
                     {data:'filingsystem.Prepared_By', title:"Prepared By"},
                     {data:'filingsystem.Destruction_Date', title:"Destruction Date"},
                     { data: "files.Web_Path",
               ...
               $('#list').on( 'click', 'tr', function () {
                ...
                fileid= listtable.api().row( this ).data().filingsystem.Id;
               });
               listeditor.on('initEdit', function ( e, json, data ) {
                  ...
                  var selectedYear = data['filingsystem']['Year'];

                  // combine list and selected
                  var combinedYears = years.concat(selectedYear);

                  // remove the duplicates
                  var uniqueYears = combinedYears.filter(function(item, pos) {
                      return combinedYears.indexOf(item) == pos;
                  });

                  listeditor.field('filingsystem.Year')
                    .update(uniqueYears)
                    .val(selectedYear);
               });
                     ...
                     <div class="active tab-pane" id="listtab">
                       ...
                       @foreach($category as $table)
                         ...
                         @if ($table->Option==$type)
                           <a href="{{ url('/filingsystem') }}/{{$table->Option}}"><button type="button" class="btn btn-success btn-small">{{$table->Option}}</button></a>
                         @else
                           <a href="{{ url('/filingsystem') }}/{{$table->Option}}"><button type="button" class="btn btn-danger btn-small">{{$table->Option}}</button></a>
                         @endif
                       ...
                       <br><br>
                        ...
                        <table id="list" class="list" cellspacing="0" width="100%" padding="30px" style="font-size: 13px;">
                            <thead>
                              @if($filingsystem)
                                <tr class="search">
                                  ...
                                  @foreach($filingsystem as $key=>$value)
                            ...
                            <tbody>
                              <?php $i = 0; ?>
                              @foreach($filingsystem as $properties)
