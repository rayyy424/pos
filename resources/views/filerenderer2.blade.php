...
<!-- {{ asset('/plugin/lightbox/css/lightbox.min.css') }}
{{ asset('/plugin/css/editor.dataTables.min.css') }} -->
...
/*Resize image because the image is disappear until the window is resized*/
#image {
    width: 100%;
    height: 100%;
...
<script src="{{ asset('/plugin/lightbox/js/lightbox-plus-jquery-2.1.4.min.js') }}"></script> <!-- Lightbox -->
...
<div class="content-wrapper">
  ...
  <!-- Content Header (Page header) -->
        <section class="content-header">
          <h1>
              {{$type}} - {{$site->{'Site ID'} }} ({{$site->{'Site Name'} }})
          </h1>
          <ol class="breadcrumb">
  ...
  <!-- Main content -->
        <section class="content">
          ...
          <div class="modal fade" id="DeleteAll" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
             <div class="modal-dialog">
               <div class="modal-content">
                 <div class="modal-header">
                   ...
                   <h4 class="modal-title" id="myModalLabel">Delete All Files</h4>

                 </div>

                 <div class="modal-body">
                     Are you sure you wish to delete all files?
                 </div>
           ...
           <div class="modal fade" id="DownloadAll" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
              <div class="modal-dialog">
                <div class="modal-content">
                  <div class="modal-header">
                    <div id="exist-alert" class="alert alert-warning alert-dismissible" style="display:none;">
                    ...
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">Download All Files</h4>

                  </div>

                  <div class="modal-body">
                      Are you sure you wish to download all files?
                  </div>


                  <div class="modal-footer">
                    <center><img src="{{ URL::to('/') ."/img/ajax-loader.gif" }}" width="50px" height="50px" alt="Loading" name='ajaxloader' id="ajaxloader"></center>
                    <a href="" id="downloadzip" name="downloadzip" target="_self"></a>
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary" onclick="downloadall()">Download</button>
                  </div>
                </div>
              </div>
            </div>

            <div class="row" id="manualuploader">
            <h4>Manual Uploader</h4>
                <form action="{{Url('/tracker/submitdocumentmanual')}}" enctype="multipart/form-data" method="POST">
                  <input type="file" id="document" name="document[]" multiple><br/>
          ...
          <div class="row" id="filecontent">
            <div class="row">
              ...
              <a data-toggle="modal" data-target="#DownloadAll"><button type="button" class="btn btn-primary">Download All</button></a>

                @if($me->Delete_File)

                  <a data-toggle="modal" data-target="#DeleteAll"><button type="button" class="btn btn-danger">Delete All</button></a>
                @endif
                ...
                <div class="col-md-2">
                  ...
                  @if(strpos($file->Web_Path,'.png') !== false || strpos($file->Web_Path,'.jpg') !== false || strpos($file->Web_Path,'.jpeg') !== false ||strpos($file->Web_Path,'.PNG') !== false || strpos($file->Web_Path,'.JPG') !== false || strpos($file->Web_Path,'.JPEG') !== false)
                      ...
                      <a class="example-image-link" href="{{ url($file->Web_Path) }}" data-lightbox="example-set" download="{{ $file->File_Name }}" title="Download"><img src="{{ url($file->Web_Path) }}" id="image" alt="Photo" width="100px" height="100%" />{{ $file->File_Name}}<a href="/salesordertemplate/{{$file->salesorderid}}" target="_blank">{{$file->SO_Number}}</a>
                        ...
                        @if($me->Delete_File)
                          |
                            <a href="{{ URL::to("/")}}/filerenderer2delete/{{$file->Id}}" title="Remove" onclick="return confirm('Are you sure you want to delete {{ $file->File_Name }}?')">Remove</a>
                        <!-- @elseif($file->UserId==$me->UserId) -->
                          <!-- |
                            <a href="{{ URL::to("/")}}/filerenderer2delete/{{$file->Id}}" title="Remove" onclick="return confirm('Are you sure you want to delete {{ $file->File_Name }}?')">Remove</a> -->
                        @endif
                      </a>
...
<script type="text/html" id="files-template">
        ...
        <li class="media" >
             <div id="deleteUpload">
            <img class="preview-img"  alt="Generic placeholder image" style="width:80px;height:60px;">
            ...
            <div>
                <p>
                <strong>%%filename%%</strong> - Status: <span class="text-muted" id="change">Waiting</span>
                </p>
    ...
    function deleteall()
    {
        ...
        $.ajax({
                    url: "{{ url('/tracker/deleteallfiles') }}",
                    method: "POST",
                    data: {
                      TrackerId:{{$trackerid}},
                      Type:"{{$type}}"
                    ...
                    success: function(response){
                      ...
                      if (response==0)
                      {
                        ...
                        var message ="Failed to delete all files!";
    ...
    function downloadall()
    {
      ...
      $.ajax({
                  url: "{{ url('/filerenderer2downloadall/') }}",
                  method: "POST",
                  data: {
                    TrackerId:{{$trackerid}},
                    Type:"{{$type}}"
                  ...
                  success: function(response){
                    ...
                    if (response==0)
                    {
                      ...
                      $('#downloadzip').attr({href  : response });
                      $('#downloadzip')[0].click();
                    }
          }
      });
