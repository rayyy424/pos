...
@section('datatable-js')
    ...
    $(function () {
			...
			columns: [
                ...
                }},
                {
                    ...
                    render:function(data,type,row,meta){
                        ...
                        if((({{$me->Quotation_Approval}} || data.material.UserId == {{$me->UserId}}) && data.request.total))
                        {
        ...
        $(".approvedTable thead input").keyup ( function () {
         ...
         oTable2=$("#rejectedTable").DataTable({
            ajax: {
				"url": "{{ asset('/Include/materialrequest.php') }}",
				"data":{
					id:{{$me->UserId}},
	          ...
	          <div class="box-body">
                    <div class="nav-tabs-custom">
                        <ul class="nav nav-tabs">
                            ...
                            <li class="active"><a href="#approved" data-toggle="tab" id="approvedtab">Approved</a></li>
                            <li><a href="#recalled" data-toggle="tab" id="recalledtab">Recalled</a></li>
                            {{-- <li><a href="#saved" data-toggle="tab" id="savedtab">Saved</a></li> --}}
                            {{-- <li><a href="#rejected" data-toggle="tab" id="rejectedtab">Rejected</a></li> --}}
                        </ul>
                        <div class="tab-content">
                            ...
                            <div class="tab-pane" id="rejected">
                                <div class="row">
                                    <div class="col-md-12">
                                        <table id="rejectedTable" cellspacing="0" width="100%" padding="30px" style="font-size: 13px;">
                                        </table>
                                    </div>
                                </div>
                            </div>
        ...
        success: function (response) {
            ...
            @if($me->Quotation_Approval)
                 filetable.row.add([
                   ...
                   ( response.data[y].Status == "Pending Approval" ? "<a style='width:unset;' class='btn btn-sm btn-primary' onclick='approvalModal("+response.data[y].Id+",`approve`)'>Approve</a> <a style='width:unset;' class='btn btn-sm btn-warning' onclick='approvalModal("+response.data[y].Id+",`reject`)'>Reject</a>":""),response.data[y].Reason != "" ? response.data[y].Reason:""
                ]).draw(false);
            @else
            filetable.row.add([
                    "",response.data[y].Type,response.data[y].Amount,"<a target='_blank' href='"+response.data[y].Web_Path+"'>"+response.data[y].File_Name+"</a>",response.data[y].Name,
                    response.data[y].Status != "" ? response.data[y].Status:"-",response.data[y].Requestor_Reason != "" ? response.data[y].Requestor_Reason:"-",response.data[y].Reason != "" ? response.data[y].Reason:"-",
                    "-",  (response.data[y].created_by == {{$me->UserId}} ? "<a onclick='removeFile("+response.data[y].Id+",this)' style='width:unset;' class='btn btn-danger btn-sm'>Remove</a>":"") +response.data[y].Reason != "" ? response.data[y].Reason:""
                ]).draw(false);
            @endif
            }                
        }
