...
@section('datatable-js')
   ...
   //                              ajax: "{{ asset('/Include/gensetinventory.php') }}",
   //                               table: "#inventorytable",
   //                               idSrc: "gensetinventory.Id",
   //                               fields: [
   //                                       {
   //                                               label: "Id",
   //                                               name: "gensetinventory.Id",
   //                                               type: "hidden"
   //                                       },
   //                                       {
   //                                               label: "Name:",
   ...
   //                       } );
		   ...
		   // 			var name = this.field('gensetinventory.name');
		   // 			var type = this.field('gensetinventory.type');
		   // 			var barcode = this.field('gensetinventory.barcode');
     ...
     //                  }
		...
		oTable=$('#inventorytable').dataTable( {
            ...
            iDisplayLength:10,
             fnInitComplete: function(oSettings, json) {
                        ...
                        api.rows().every( function (){
                       	var d = this.data();
                        total = d.inventorypricehistory.price * d.gensetinventory.qty_balance;
                        grand = grand + total;
                    	});
                        console.log(grand);
			...
			columns: [
				...
				{ data:'companies.Company_Name',title:'Supplier', editField:'gensetinventory.supplier'},
				// { data:'gensetinventory.branch',title:'Branch'},
				...
				{ data:'gensetinventory.status',title:'Status',
					render: function ( data, type, full, meta ) {
						if(full.gensetinventory.status == "Active")
						{
				...
				{ title:'Total Amount(RM)',
					render: function ( data, type, full, meta ) {
						var price = full.inventorypricehistory.price;
						var qty = full.gensetinventory.qty_balance;
						var total = price * qty;
				...
				{ data:null, title:"Action",
					render: function ( data, type, full, meta ) {
							return '<a  href="{{ url("genset/inventorydetails")}}/'+full.gensetinventory.Id+'"" target="_blank"><button class="btn btn-default btn-xs" title="Details" style="width:unset"><i class="fa fa-edit"></i></button></a>'+ 
								...
								// '<a  href="{{ url("genset/inventorydetails")}}/'+full.gensetinventory.Id+'"" target="_blank"><button class="btn btn-default btn-xs" title="Stock-out" style="width:unset"><i class="fa fa-minus-circle"></i></button></a>'+
								'<button class="btn btn-default btn-xs" title="Delete" style="width:unset" onclick="OpenDeleteDialog('+full.gensetinventory.Id+')"><i class="fa fa-close"></i></button></a>'
							;
						}
				}
			],
            ...
            //                          .create('Create New Item', {
								    ...
								    // }, true )
            //                          .set('gensetinventory.status',"Active")
		...
		$('#inventorytable').on( 'click', 'tr', function () {
			...
			// var rowid = oTable.api().row( this ).data().gensetinventory.Id;
		});
...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1>GENSET Inventory Management<small>GENSET</small></h1>

		<ol class="breadcrumb">
			<li><a href="{{ url('/home') }}"><i class="fa fa-dashboard"></i>Home</a></li>
			<li><a href="#">GENSET</a></li>
			<li class="active">GENSET Inventory Management</li>
		</ol>
	</section>
     ...
     <div id="warning" class="alert alert-danger">
                	...
                	<form id="newitemform">
                  	<label>Name</label>
	                  	...
	                  	<select name="type" class="form-control" required="">
	                  		<option></option>
	                  		@foreach($gensetinventorytype as $git)
	                  	...
	                  	<select name="model" class="form-control" required="" >
	                  		<option></option>
	                  		@foreach($gensetinventorymodel as $gim)
      ...
      <div class="modal modal-success fade" id="update-alert">
		...
		<div class="modal fade" id="importModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<form action="{{ url('importgensetinventory') }}" method="POST" enctype="multipart/form-data">
						{{ csrf_field() }}
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
					    ...
					    <div class="modal-body">
					    	<a href="{{url('exportGensetInventory')}}"><i class="fa fa-file-excel-o"> Export Template</i></a>
					        <div class="form-group">
	            ...
	            <div class="col-md-12">
	            	...
	            	<table id="inventorytable" class="inventorytable" cellspacing="0" width="100%" padding="30px" style="font-size: 13px; white-space: pre-line;">
		              	<thead>
                            ...
                            <tbody>
                                <?php $i = 0; ?>
                                @foreach($gensetinventory as $delivery)
 ...
 function OpenModal(row)
  {
 	  ...
 	  $('#item_name').val(oTable.api().row(row).data().gensetinventory.name);
 	  $('#qty_balance').val(oTable.api().row(row).data().gensetinventory.qty_balance)
 	  $("#StockId").val(oTable.api().row(row).data().gensetinventory.Id);
	...
	function refresh()
{
      ...
      // if(b == "")
      // {
      // 	window.location.href ="{{ url('/genset/inventory') }}/"+arr[0]+"/"+arr[1];
      // }
      // else
      // {
      // 	window.location.href ="{{ url('/genset/inventory') }}/"+arr[0]+"/"+arr[1] + b;
      // }

      window.location.href ="{{ url('/genset/inventory') }}/"+ b;
}
